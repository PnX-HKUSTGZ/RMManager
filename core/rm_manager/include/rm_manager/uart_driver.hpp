...
public:
     ...
     */
    SerialCommunicator(
        const std::string & port_name, unsigned int baud_rate,
    ...
    ~SerialCommunicator()
    {
        ...
        if (serial_port_.is_open()) {
            ...
            if (ec) {
                std::cerr << "Error closing serial port in destructor: " << ec.message() <<
                    std::endl;
            } else {
    ...
    bool openPort()
    {
        ...
        if (ec) {
            std::cerr << "Failed to open serial port " << port_name_ << ": " << ec.message() <<
                std::endl;
        ...
        try {
          ...
          boost::asio::serial_port_base::flow_control::none));
            ...
            std::cerr << "Failed to set serial port options for " << port_name_ << ": " <<
                ex.what() <<
                std::endl;
        ...
        std::cout << "Serial port " << port_name_ << " opened at " << baud_rate_ << " baud." <<
            std::endl;
        return true;
    }
    ...
    bool closePort()
    {
        ...
        serial_port_.close(ec);
        if (ec) {
            std::cerr << "Error closing serial port " << port_name_ << ": " << ec.message() <<
                std::endl;
            error_occurred_.store(true); // 标记错误
            return false;
        }
