  ...
  : Node("rm_custom_controller_state", options)
{
    ...
    // 填充参数
    arm_state_topic_ = params_.arm_state_topic;
    joint_names_ = params_.joint_names;
    joint_reverse_ = params_.joint_reverse;
    ...
    if (joint_names_.size() != JOINT_NUM) {
        ...
        joint_names_.size());
        throw std::runtime_error("Joint names size is invalid");
    }
    if (joint_reverse_.size() != JOINT_NUM) {
        RCLCPP_ERROR(this->get_logger(), "Joint reverse flags size must be %d, got %zu", JOINT_NUM,
        joint_reverse_.size());
        throw std::runtime_error("Joint reverse flags size is invalid");
    }

    // 输出所有参数
    RCLCPP_INFO(this->get_logger(), "Arm state topic: %s", arm_state_topic_.c_str());
    RCLCPP_INFO(this->get_logger(), "Joint names:");
    for (const auto & name : joint_names_) {
        RCLCPP_INFO(this->get_logger(), "  %s", name.c_str());
    }
    RCLCPP_INFO(this->get_logger(), "Joint reverse flags:");
    for (size_t i = 0; i < JOINT_NUM; ++i) {
        RCLCPP_INFO(this->get_logger(), "  j%zu: %s", i, joint_reverse_[i] ? "true" : "false");
    }
    ...
    if (channel_mapping_.size() != 4) {
        RCLCPP_ERROR(this->get_logger(), "channel_mapping size must be 4, got %zu",
        channel_mapping_.size());
        throw std::runtime_error("channel_mapping size must be 4");
    }
    if (channel_max_.size() != 4) {
        RCLCPP_ERROR(this->get_logger(), "channel_max size must be 4, got %zu",
              channel_max_.size());
    ...
    if (enable_chassis_cmd_) {
        ...
        RCLCPP_INFO(this->get_logger(),
              "Channel max values - ch0:%.2f, ch1:%.2f, ch2:%.2f, ch3:%.2f",
    ...
    // 创建发布者
    arm_state_pub_ = this->create_publisher<sensor_msgs::msg::JointState>(arm_state_topic_, 10);
    if (enable_chassis_cmd_) {
        chassis_cmd_pub_ =
          this->create_publisher<geometry_msgs::msg::TwistStamped>(chassis_cmd_topic_,
        10);
    }
    ...
    RCLCPP_INFO(this->get_logger(),
          "RmCustomControllerState node initialized with watchdog enabled");

}

void RmCustomControllerState::ref_topic_callback(
    const rm_message::msg::CustomController::SharedPtr msg)
{
    if(msg == nullptr) {
    ...
    if (msg->data.size() != sizeof(ControlData)) {
...
void RmCustomControllerState::ref_topic_callback_original(const rm_message::msg::CustomController::SharedPtr msg){
    ...
    rclcpp::Time now = this->now();
    std::memcpy(&control_data, msg->data.data(), sizeof(ControlData));
    ...
    // 输出 joint_state_msg
    RCLCPP_DEBUG(this->get_logger(), "Publishing arm joint states:");
    for (size_t i = 0; i < JOINT_NUM; ++i) {
        RCLCPP_DEBUG(this->get_logger(), "  %s: %.4f", joint_names_[i].c_str(),
